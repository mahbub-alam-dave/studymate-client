...
const SubmittedAssignmentCards = ({mySubmittedAssignment}) => {
    ...
    if(submittedAssignments.length === 0) {
        return <EmptyComponents message={"You haven't submitted any assignments"} />
    }
    return (
        <div className='py-12 px-4 sm:px-5 md:px-6 flex flex-col gap-8 max-w-[1440px] mx-auto'>
            <h2 className='text-2xl md:text-3xl font-bold text-center text-[#FF3F33] dark:text-gray-200'>My Submitted Assignments</h2>
            <div className='flex flex-col gap-6 w-full'>
           {
            submittedAssignments.map((submittedAssignment, index) => {
                return(
                    <div key={submittedAssignment._id} className='w-full shadow-gray-400 dark:shadow-[#000814] bg-gradient-to-l from-[#A8F1FF] to-[#00b4d8] dark:bg-gradient-to-bl dark:from-[#03045e] dark:to-[#000814] text-gray-200 border border-white dark:border-[#03045e] shadow-sm p-4 sm:p-6 flex flex-col gap-2 rounded-xl'>
